...
import {
  ...
  userNewDataEdit,
  userNewDataUpload,
} from '../../helpers/userNewData';
import { USER_ROLES, UserRoleTypes } from '../../types/types';
...
export default function Admin() {
  ...
  const uploadUserDataHandler = async ({
    ...
    const { updatedUserData, error } = await userNewDataUpload({
      e,
      userEmail,
      updatedField,
    });
  ...
  const newUserDataEditHandler = async ({
    ...
    if (index && originalData.users) {
      const updatedObject = userNewDataEdit({ newValue, user, updatedField });
    ...
    <div className="w-full">
      ...
      <div className="w-full">
        {newDataAllUsers.users.map((user) => (
          <Disclosure key={user._id}>
            {({ open }) => (
              <>
                <Disclosure.Button
                  className={`${
                    open && 'font-semibold'
                  } mt-3 flex w-full justify-between rounded-t-md bg-gray-200 px-3 py-2 first:mt-0`}
                >
                  {user.username}
                  <ChevronRightIcon
                    width={24}
                ...
                <Disclosure.Panel className="space-y-4 rounded-b-md bg-gray-50 p-3 ">
                    ...
                    <div className="flex flex-wrap justify-between sm:justify-start">
                      ...
                      <fieldset className="flex flex-col pr-3">
                        ...
                        <input
                          ...
                          className={`${
                            newDataAllUsers.error === UpdateNewDataType.Email &&
